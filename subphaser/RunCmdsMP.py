...
try:
	...
	if "DRMAA_LIBRARY_PATH" in format(e):
		...
		pass
# logger.info('No DRMAA (see https://github.com/pygridtools/drmaa-python), \
...
class Grid(object):
	def __init__(self, cmd_list=None,
		...
		if self.cmd_list is not None:
			...
			self.out_path = out_path  # The path to a file for job's stdout.
			self.err_path = err_path  # The path to a file for job's stderr
	...
	def submit(self):
		...
		for curjob in joblist:
			...
			task_id, status = retval.jobId, (not retval.hasExited) + \
				retval.exitStatus
	...
	def which_grid(self):
		...
		if 'OGS/GE' in name or 'SGE' in name:
			...
			logger.warn(
				'Please provide your grid system `{}` to auther'.format(name))
		return 'sge'
...
def run_tasks(cmd_list, tc_tasks=None, mode='grid', grid_opts='', cpu=1, mem='1g', cont=1,
	...
	while True:
		...
		if mode == 'grid':
			...
			logger.info('reset tc_tasks to {} by min({})'.format(
				tc_tasks, avail_tasks))
			...
			avail_tasks = [tc_tasks0, avail_cpu(
				cpu), avail_mem(mem), len(cmd_list)]
			tc_tasks = min(avail_tasks)
			logger.info('reset tc_tasks to {} by {}'.format(
				tc_tasks, avail_tasks))
		...
		if ntry >= retry or not uncmp:
			logger.info(
				'finished with {} commands uncompleted'.format(len(uncmp)))
			break
		cmd_list = uncmp
	# close
	if close_cmp:
...
def mem2float(mem):
	...
	try:
		num, unit = re.compile(
			r'(\d+\.?\d*)([kmgt]?)', re.I).match(mem).groups()
		return float(num) * d_mem[unit.lower()]
	except AttributeError:
		raise AttributeError(
			'Illegal MEMORY string `{}` (legal: 2g, 100m, 0.3t).'.format(mem))
...
def run_cmd(cmd, log=False, logger=None, fail_exit=False, **kargs):
	...
	output = map(lambda x: x.decode(), output)
	output = tuple(output)
	status = job.poll()
...
def pp_run(cmd_list, processors='autodetect', **kargs):
	...
	jobs = [job_server.submit(run_cmd, (cmd,), (), ('subprocess',))
			for cmd in cmd_list]
...
def pp_func(func, lst, args=(), funcs=(), libs=(), processors='autodetect'):
	ppservers = ()
	job_server = pp.Server(processors, ppservers=ppservers)
	jobs = [job_server.submit(func, add_args(
		value, args), funcs, libs) for value in lst]
	return jobs
...
def run_job(cmd_file=None, cmd_list=None, by_bin=1, tc_tasks=8, mode='grid',
			grid_opts='-tc {tc}', cont=1, fail_exit=True,
